...
export function Countdown({endsAt,serverTime,compact=false}:{endsAt:string;serverTime:string;compact?:boolean}) {
  ...
  return <span className={`countdown ${remaining<3600000?'urgent':''}`} aria-label={remaining===0?'Auction closing':`Time remaining ${text}`}>{remaining===0?'Closing…':text}{!compact && days===0 && <span className="timer-units"> HRS : MIN : SEC</span>}</span>;
}
